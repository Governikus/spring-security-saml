...
class LogoutObjectTests
{
  ...
  void requestFromXml()
  {
    LogoutRequest request = (LogoutRequest)saml.resolve(EXAMPLE_REQUEST,
  ...
  void requestToXml()
  {
    ...
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutRequest").iterator().next(), "Destination", equalTo(destination));
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutRequest").iterator().next(), "ID", equalTo("request-id"));
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutRequest").iterator().next(), "Version", equalTo("2.0"));
    ...
    assertNodeAttribute(getNodes(xml, "//ds:DigestMethod").iterator().next(), "Algorithm", equalTo(SHA512.toString()));

    assertNodeCount(xml, "//ds:SignatureMethod", 1);
    assertNodeAttribute(getNodes(xml, "//ds:SignatureMethod").iterator().next(),
                        "Algorithm",
                        equalTo(RSA_SHA256.toString()));

    assertNodeCount(xml, "//samlp:LogoutRequest/saml:NameID", 1);
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutRequest/saml:NameID").iterator().next(),
                        "NameQualifier",
                        equalTo(issuer));
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutRequest/saml:NameID").iterator().next(),
                        "SPNameQualifier",
    ...
    Exception expected = assertThrows(SignatureException.class,
                                      // using the wrong key
                                      () -> saml.validateSignature(saml.resolve(xml, null, null),
                                                                   Arrays.asList(SP_RSA_KEY.getSigningKey("wrong"))));
    assertThat(expected.getMessage(),
               equalTo("Signature validation against a org.opensaml.saml.saml2.core.impl.LogoutRequestImpl object failed "
                       + "using 1 key."));

  }

  @Test
  void responseFromXml() throws Exception
  {
    LogoutResponse response = (LogoutResponse)saml.resolve(EXAMPLE_RESPONSE,
                                                           Arrays.asList(RSA_TEST_KEY.getSigningKey("test")),
                                                           null);
    ...
    assertThat(signature.getCanonicalizationAlgorithm(),
               equalTo(CanonicalizationMethod.ALGO_ID_C14N_EXCL_OMIT_COMMENTS));
  }

  @Test
  void responseToXml() throws Exception
  {
    LogoutResponse response = new LogoutResponse().setInResponseTo("in-response-to")
    ...
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutResponse").iterator().next(), "Destination", equalTo(destination));
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutResponse").iterator().next(), "ID", equalTo("response-id"));
    ...
    assertNodeCount(xml, "//ds:DigestMethod", 1);
    assertNodeAttribute(getNodes(xml, "//ds:DigestMethod").iterator().next(), "Algorithm", equalTo(SHA256.toString()));
    ...
    assertNodeAttribute(getNodes(xml, "//samlp:LogoutResponse/samlp:Status/samlp:StatusCode").iterator().next(),
                        "Value",
                        equalTo(StatusCode.SUCCESS.toString()));
    assertThat(getNodes(xml, "//samlp:LogoutResponse/samlp:Status/samlp:StatusMessage").iterator()
                                                                                       .next()
                                                                                       .getTextContent(),
    ...
    Exception expected = assertThrows(SignatureException.class,
                                      ...
                                      () -> saml.validateSignature(saml.resolve(xml, null, null),
                                                                   Arrays.asList(SP_RSA_KEY.getSigningKey("wrong"))));
    assertThat(expected.getMessage(),
               equalTo("Signature validation against a org.opensaml.saml.saml2.core.impl.LogoutResponseImpl object failed "
                       + "using 1 key."));
