 ...
 * Page 13, Line 268
 */
public class EntityDescriptor<T extends EntityDescriptor<T>> extends ImplementationHolder
{
  ...
  EntityDescriptor(EntityDescriptor<T> other)
  {
    this.id = other.id;
  ...
  public List<SsoProvider<?>> getSsoProviders()
  {
    ...
    {
      getProviders().stream().filter(SsoProvider.class::isInstance).forEach(p -> result.add((SsoProvider<?>)p));
    }
    return result;
  }
  ...
  public T setSigningKey(SigningKey signingKey, AlgorithmMethod algorithm, DigestMethod digest)
  {
