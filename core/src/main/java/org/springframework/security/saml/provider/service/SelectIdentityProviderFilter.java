...
public class SelectIdentityProviderFilter extends
  SamlFilter<HostedServiceProviderService, LocalServiceProviderConfiguration, ServiceProviderMetadata, IdentityProviderMetadata, ExternalIdentityProviderConfiguration>
{
  ...
  private static final Log log = LogFactory.getLog(SelectIdentityProviderFilter.class);

  private final RequestMatcher requestMatcher;

  private String selectTemplate = "/templates/spi/select-provider.vm";

  public SelectIdentityProviderFilter(SamlProviderProvisioning<HostedServiceProviderService, LocalServiceProviderConfiguration, ServiceProviderMetadata, IdentityProviderMetadata, ExternalIdentityProviderConfiguration> provisioning)
  {
    this(provisioning, new SamlRequestMatcher<>(provisioning, "select"));
  }
    ...
    throws ServletException, IOException
  {
    if (requestMatcher.matches(request))
    {
      ...
      configuration.getProviders().stream().forEach(p -> {
        try
        {
          ModelProvider mp = new ModelProvider().setLinkText(p.getLinktext())
                                                .setRedirect(getDiscoveryRedirect(provider, request, p));
          providers.add(mp);
        }
        catch (Exception x)
        {
          log.debug(format("Unable to retrieve metadata for provider:%s with message:%s",
                           p.getMetadata(),
                           x.getMessage()));
        }
      });
      ...
      processHtml(request, response, selectTemplate, model);
    }
    else
    {
  ...
  {
    UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(provider.getConfiguration().getBasePath());
