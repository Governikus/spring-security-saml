...
public class HostedServiceProviderService extends
  AbstractHostedProviderService<LocalServiceProviderConfiguration, ServiceProviderMetadata, IdentityProviderMetadata, ExternalIdentityProviderConfiguration>
  implements ServiceProviderService
{
                                      ...
                                      AuthenticationRequestEnhancer authnRequestEnhancer)
  {
    ...
    this.authnRequestEnhancer = ofNullable(authnRequestEnhancer).orElseGet(() -> authenticationRequest -> authenticationRequest);
  }
  ...
  {
    Metadata<IdentityProviderMetadata> metadata = (Metadata<IdentityProviderMetadata>)getTransformer().fromXml(data,
                                                                                                               null,
                                                                                                               null);
    ...
    else
    {
      ...
      providers = providers.stream().filter(IdentityProvider.class::isInstance).collect(Collectors.toList());
  ...
  public IdentityProviderMetadata getRemoteProvider(Saml2Object saml2Object)
  {
    ...
    {
      return getRemoteProvider((ManageNameIDRequest)saml2Object);
    }
    else
    {
  ...
  public IdentityProviderMetadata getRemoteProvider(ManageNameIDRequest request)
  {
  ...
  public AuthenticationRequest authenticationRequest(IdentityProviderMetadata idp)
  {
    ...
    AuthenticationRequest request = new AuthenticationRequest()
                                                               ...
                                                               .setAssertionConsumerService(getPreferredEndpoint(sp.getServiceProvider()
    ...
    {
      request.setNameIdPolicy(new NameIdPolicy<>(idp.getDefaultNameId(), sp.getEntityAlias(), true));
    }
    else if (idp.getIdentityProvider().getNameIds().size() > 0)
    {
      request.setNameIdPolicy(new NameIdPolicy<>(idp.getIdentityProvider().getNameIds().get(0), sp.getEntityAlias(),
                                                 true));
    }
