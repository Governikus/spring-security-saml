...
public class ExternalProviderConfiguration<T extends ExternalProviderConfiguration<T>> implements Cloneable
{
  ...
  public List<SigningKey> getVerificationKeyData()
  {
    return getVerificationKeys().stream()
                                .map(s -> new SigningKey("from-config-" + UUID.randomUUID().toString(), s))
