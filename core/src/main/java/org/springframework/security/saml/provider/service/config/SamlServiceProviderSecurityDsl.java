...
public class SamlServiceProviderSecurityDsl extends AbstractHttpConfigurer<SamlServiceProviderSecurityDsl, HttpSecurity>
{
  ...
  private SamlServerConfiguration configuration = new SamlServerConfiguration().setNetwork(new NetworkConfiguration().setConnectTimeout(5000)
                                                                               ...
                                                                               .setServiceProvider(new LocalServiceProviderConfiguration().setPrefix(prefix)
  ...
  public void configure(HttpSecurity http) throws Exception
  {
    ...
    {
      SamlServiceProviderServerBeanConfiguration beanConfig = context.getBean(SamlServiceProviderServerBeanConfiguration.class);
      ...
      http.addFilterAfter(samlConfigurationFilter, BasicAuthenticationFilter.class)
          .addFilterAfter(metadataFilter, samlConfigurationFilter.getClass())
          .addFilterAfter(spAuthenticationRequestFilter, metadataFilter.getClass())
  ...
  {
    configuration.getServiceProvider().setDefaultSigningAlgorithm(signAlgorithm).setDefaultDigest(signDigest);
    return this;
  }

  public SamlServiceProviderSecurityDsl singleLogout(boolean enabled)
  {
    configuration.getServiceProvider().setSingleLogoutEnabled(enabled);
    return this;
  }

  public SamlServiceProviderSecurityDsl nameIds(List<NameId> nameIds)
  {
    configuration.getServiceProvider().setNameIds(nameIds.stream().collect(Collectors.toList()));
    return this;
  }

  public SamlServiceProviderSecurityDsl rotatingSigningKeys(RotatingSigningKeys signingKeys)
  {
    configuration.getServiceProvider().setSigningKeys(signingKeys);
    return this;
  }

  public SamlServiceProviderSecurityDsl rotatingeEncryptionKeys(RotatingEncryptionKeys encryptionKeys)
  {
    configuration.getServiceProvider().setEncryptionKeys(encryptionKeys);
    return this;
  }

  public SamlServiceProviderSecurityDsl identityProvider(ExternalIdentityProviderConfiguration idp)
  {
    configuration.getServiceProvider().getProviders().add(idp);
    return this;
  }
