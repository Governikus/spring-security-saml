  ...
  extends AbstractHttpConfigurer<SamlIdentityProviderSecurityDsl, HttpSecurity>
{
  ...
  private SamlServerConfiguration configuration = new SamlServerConfiguration().setNetwork(new NetworkConfiguration().setConnectTimeout(5000)
                                                                                                                     .setReadTimeout(10000))
                                                                               .setIdentityProvider(new LocalIdentityProviderConfiguration().setPrefix(prefix)
  ...
  public void configure(HttpSecurity http) throws Exception
  {
    ...
    serverConfig.transfer(configuration);

    if (useStandardFilterConfiguration)
    {
      SamlIdentityProviderServerBeanConfiguration beanConfig = context.getBean(SamlIdentityProviderServerBeanConfiguration.class);
      Filter samlConfigurationFilter = beanConfig.samlConfigurationFilter();
      Filter metadataFilter = beanConfig.idpMetadataFilter();
      Filter idpValidationFilter = beanConfig.idpValidationFilter();
      ...
      http.addFilterAfter(samlConfigurationFilter, SecurityContextPersistenceFilter.class)
          .addFilterAfter(metadataFilter, samlConfigurationFilter.getClass())
          .addFilterAfter(idpValidationFilter, metadataFilter.getClass())
          .addFilterAfter(idpInitiateLoginFilter, metadataFilter.getClass())
          .addFilterAfter(idpAuthnRequestFilter, idpInitiateLoginFilter.getClass())
          .addFilterAfter(idpLogoutFilter, idpAuthnRequestFilter.getClass())
          .addFilterAfter(beanConfig.idpSelectServiceProviderFilter(), idpLogoutFilter.getClass());
    }
  }
  ...
  {
    configuration.getIdentityProvider().setDefaultSigningAlgorithm(signAlgorithm).setDefaultDigest(signDigest);
    return this;
  }

  public SamlIdentityProviderSecurityDsl signAssertions(boolean sign)
  {
    configuration.getIdentityProvider().setSignAssertions(sign);
    return this;
  }

  public SamlIdentityProviderSecurityDsl wantRequestsSigned(boolean sign)
  {
    configuration.getIdentityProvider().setWantRequestsSigned(sign);
    return this;
  }
  ...
  {
    configuration.getIdentityProvider().setSingleLogoutEnabled(enabled);
    return this;
  }

  public SamlIdentityProviderSecurityDsl nameIds(List<NameId> nameIds)
  {
    configuration.getIdentityProvider().setNameIds(nameIds.stream().collect(Collectors.toList()));
    return this;
  }

  public SamlIdentityProviderSecurityDsl rotatingSigningKeys(RotatingSigningKeys signingKeys)
  {
    configuration.getIdentityProvider().setSigningKeys(signingKeys);
    return this;
  }

  public SamlIdentityProviderSecurityDsl rotatingEncryptionKeys(RotatingEncryptionKeys encryptionKeys)
  {
    configuration.getIdentityProvider().setEncryptionKeys(encryptionKeys);
    return this;
  }

  public SamlIdentityProviderSecurityDsl serviceProvider(ExternalServiceProviderConfiguration sp)
  {
    configuration.getIdentityProvider().getProviders().add(sp);
    return this;
  }
  ...
  {
    useStandardFilterConfiguration = enable;
    return this;
  }
