...
public class DefaultMetadataCache implements SamlMetadataCache
{
  ...
  public DefaultMetadataCache(Clock time, RestOperations validatingNetwork, RestOperations nonValidatingNetwork)
  {
    cache = new TimebasedMap<>(time);
    cache.setFrequencyIntervalMills(TimeUnit.MINUTES.toMillis(2));
    cache.setExpirationTimeMills(TimeUnit.MINUTES.toMillis(10)); // 10 minutes default for hits
    misses = new TimebasedMap<>(time);
    misses.setFrequencyIntervalMills(TimeUnit.MINUTES.toMillis(2));
    misses.setExpirationTimeMills(TimeUnit.MINUTES.toMillis(5)); // 5 minutes default for misses
  ...
  public DefaultMetadataCache setCacheHitDurationMillis(long cacheHitDurationMillis)
  {
    ...
    cache.setFrequencyIntervalMills(Math.round(cacheHitDurationMillis / 2.0d));
    return this;
  }
  ...
  public DefaultMetadataCache setCacheMissDurationMillis(long cacheMissDurationMillis)
  {
    ...
    misses.setFrequencyIntervalMills(Math.round(cacheMissDurationMillis / 2.0d));
    return this;
  }
  ...
  public byte[] getMetadata(String uri, boolean skipSslValidation)
  {
    ...
    if (data == null)
    {
      ...
      {
        SamlProviderNotFoundException ex = new SamlProviderNotFoundException(format("Unable to download SAML metadata[%s]",
                                                                                    uri),
                                                                             x);
        misses.put(uri, ex);
        throw ex;
