...
class TimebasedMap<K, V> implements Map<K, V>
{
  ...
  private long expirationTimeMills = TimeUnit.MINUTES.toMillis(10);

  private long frequencyIntervalMills = TimeUnit.SECONDS.toMillis(30);
  ...
  private void scanAndRemove()
  {
    ...
    if (remove)
    {
      ...
      possibleRemovals.stream().forEach(this::remove);
    }
  }
  ...
  {
    scanAndRemove();
    MapEntry<V> entry = new MapEntry<>(time, value);
  ...
  {
    scanAndRemove();
    m.entrySet().stream().forEach(e -> map.put(e.getKey(), new MapEntry<>(time, e.getValue())));
  }
  ...
  {
    scanAndRemove();
    return map.entrySet().stream().map(e -> e.getValue().getValue()).collect(toList());
  }
